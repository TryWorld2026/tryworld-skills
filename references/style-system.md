...
### 动效丰富层与安全护栏（在原有三签名之上加量，不加乱）

在保持三种签名与上述节奏的前提下，强制以下丰富度下限：

- **动效密度**：每 3-5 秒至少一个可见动效（含环境动效）；每 8-15 秒一个重音动画（数字、结论、强调句、图表入场）；禁止连续 3 秒以上画面静止。
- **环境动效（ambient）**：每场至少 1 个——纸纹浮尘缓慢漂移、朱红光晕呼吸（opacity 0.06-0.12 循环）、背景超大褪色字轻微漂移、卡片 0.5-1° 微旋或 2-4px 浮动、整体 scale 1.0→1.06 缓推。循环动效一律有限 repeat（按场景时长计算次数），每场换一种，禁止全场同一种。
- **视觉层次**：每场至少 3 层（背景光晕/超大褪色字/墨线面板 + 内容层 + 分隔线/标签/数据条等强调层）与 2 个焦点；背景禁止留白死区。
- **动态字幕**：字幕 karaoke 逐词扫色（当前词纸面/朱红），按句能量分级（普通句 subtle、重音句 color pop + 轻微 scale、高潮句 slam/elastic/3D 入场）；关键词加 marker 强调（马克笔/手绘圈/划线），每 2-3 句换一种模式。

安全护栏（防重叠/防诡异/防打架，违反即返工）：

- **空间**：字幕区（底部约 200px 高）与印章区（右上角约 220px）为动画禁区；字幕与内容动画分属独立层级，任何内容动画不得进入这两个区域。
- **时间**：入场错峰（stagger），同一时刻同时运动的元素不超过 3 个；同一场景入场方式不重复。
- **运动**：只用风格内变换（x/y/scale/opacity/微旋转 3-6°）；每条动画必须有语义（入场/强调/数据/转场/ambient 五选一）；禁止无意义旋转乱飞、闪烁、颜色突变、无来源缩放、机械匀速。
- **核验**：`inspect --strict` 之外，用 `inspect --at` 对动画中间帧采样，确认无重叠、无溢出、无遮挡。
...
- **字幕与画面分工（硬性）**：画面动画文字负责展示核心内容（标题、数据、结论、关键词），字幕负责跟随口播；同一信息同屏只呈现一次——画面已展示"42 倍""40 亿美元"等核心内容时，该句字幕可隐藏或只显示补充信息，禁止上下镜像式重复。
- 强制：主视频全程烧录字幕（与配音同步）；画面动画已展示同句核心内容时，字幕省略/精简该句属于正常分工。
- 字幕与画面不重复：同一时刻同一信息只呈现一次——画面动画文字（大字/数据卡/图表）用缩写或图表化表达（如 `42×`、`$40亿`、`150`），完整口播句由字幕承担；禁止画面文字与字幕逐字重复同一文案（品牌签名结尾除外）。
...
- **朱红印章**：右上角**全程常驻显示**——朱红"试界原创"印章（`assets/seal.svg`），盖章动画入场（scale 1.6→1 + back.out）。印章是防伪标识也是品牌记忆点。
...
封面与视频使用**两套视觉语言**：视频是浅纸面，封面是深墨海报。平台按"封面与视频画面相似度"判定截图，两套语言可避免封面被识别为截图；品牌一致性由固定元素承担（"试界TryWorld · 纸上算法"、朱红印章、思源宋体）。
