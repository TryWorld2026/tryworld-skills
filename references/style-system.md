...
- 每 3-5 秒至少一个可见动效（含环境动效）；每 8-15 秒一个重音动画（数字、结论、强调句、图表入场）；**禁止连续 3 秒以上画面静止**。
- 每个场景三阶段：build（元素分 2-4 批入场，禁止一次全出）→ breathe（内容可见 + 一个环境动效）→ resolve（转场接管）。
- 同一场景内入场方向/缓动/时长必须组合变化，同一种入场方式连续出现不得超过 2 次；至少使用 2 种签名缓动。

### 环境动效（ambient，每场至少 1 个）

- 纸纹浮尘微粒缓慢漂移、朱红光晕呼吸（opacity 0.06-0.12 循环）、背景超大褪色字样轻微漂移、卡片 0.5-1° 微旋或 2-4px 浮动、整体 scale 1.0→1.06 缓推（镜头感）。
- 循环动效一律用有限 repeat（按场景时长计算次数），禁止无限循环；ambient 每场换一种，禁止全场同一种。

### 视觉层次（每场至少 3 层 + 2 焦点）

- 背景层：光晕/超大褪色字/墨线面板，背景禁止留白死区。
- 内容层：标题/正文/图片/图表。
- 强调层：分隔线、标签、数据条、手绘圈画（分隔线用 scaleX 0→1 生长入场）。
- 每场至少 2 个焦点让视线有路径；禁止单一文字块悬浮空场。

### 动态字幕与文字强调（强制）

- 字幕用 karaoke 逐词扫色（当前词纸面/朱红），按句能量分级：普通句 subtle、重音句 color pop + 轻微 scale、高潮句可 slam/elastic/3D 入场（遵循 hyperframes dynamic-techniques）。
- 关键词加 marker 强调：马克笔扫过/手绘圈/划线/涂鸦（hyperframes css-patterns），每 2-3 句换一种模式，禁止全场同一种。
- 数字/代码/专有名词：等宽字体 + 朱红 + 盖章或计数入场。

### 音频响应（配音驱动，重音处强制）

- 用 gsap skill 的 `scripts/extract-audio-data.py` 从配音提取音频数据（8-16 bands），在数据/结论/重音句处做：bass→scale 脉冲（文字 3-6%，非文字 10-30%）、treble→glow、rms→背景明度呼吸。
- 确定性：预提取数据 + 逐帧 `tl.call` 采样，禁止 Web Audio、禁止随机。

### 转场

- 场景间必须转场：默认墨染，章节级可换 reveal/wipe/交叉溶解（hyperframes transitions catalog），禁止硬切；转场即退场（除末场外不做退场动画）。
- 章节切换前留 0.3 秒纸面空场。

### 数据图表

- 动态图表必须带入场 + 数值动画（计数/生长/描画），禁止静态图表直接摆放（见"动态图表与可视化"章节）。
