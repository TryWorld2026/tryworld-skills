...
封面必须独立创作：不截取视频画面、不复用视频中的任何图片/截图/帧素材（平台按整图感知相似度判定截图，仅换底色不够）；封面采用**深墨海报**视觉（深墨底 + 朱红光晕 + 浅色大字 + 深色卡片），与视频浅纸面画面刻意区分。制作两个独立静态构图：
...
- 平台仍提示封面疑似视频截图：检查封面是否复用了视频里的图片/截图/帧——即使换了底色也可能被感知哈希命中；封面素材必须与视频完全独立。
