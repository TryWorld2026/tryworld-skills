...
5. **涨粉 ← 价值认同**：结尾**必须使用默认结尾**——"我是试界TryWorld，持续把 AI 讲清楚，让每个普通人都看得懂、用得上。觉得这条视频有用，点个赞，也欢迎关注，谢谢大家，下期见。"；让观众因认同价值而关注。
...
每个章节 1-3 个场景：章节标题卡、讲解场景、数据卡/图片场景。数据点/对比/流程优先规划为**动态图表场景**（计数/柱状/折线/进度环/流程），避免大段静态文字配解说。规划动效密度：**每 3-5 秒一个可见动效、每 8-15 秒一个重音，每场声明 1 个 ambient 环境动效；连续静止不得超过 3 秒**。
...
- 动效：按 style-system.md 动效系统执行——每场 ≥3 层视觉、≥2 焦点、1 个 ambient；入场方向/缓动同场不重复；动态字幕 karaoke + marker 强调；数据/结论处加音频响应（`gsap` skill 的 `scripts/extract-audio-data.py` 提取配音数据）。
...
4. **风格与动效**：4 色板无越界色、字体合规（无被禁字体）、三种签名动效 + 丰富度达标——无连续 3 秒静止、每场有 ambient 与 ≥3 层视觉、动态字幕与音频响应就位；场景间有墨染转场、无硬切。
