...
每个章节 1-3 个场景：章节标题卡、讲解场景、数据卡/图片场景。数据点/对比/流程优先规划为**动态图表场景**（计数/柱状/折线/进度环/流程），避免大段静态文字配解说。规划动效丰富度：**每 3-5 秒一个可见动效、每 8-15 秒一个重音、每场 1 个 ambient、同屏并发元素 ≤3**，并标注动效禁区（字幕区/印章区）。
