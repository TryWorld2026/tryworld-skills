...
`tryworld-paper-video` 是试界TryWorld 的视频生产 Skill：输入一篇中文 AI 口播稿（可附图片），自动产出：

- **横屏主视频**（16:9，时长不限）—— 纸上算法风格，真云希配音，句级字幕同步
- **横版封面 + 竖版封面**（1920x1440 横 4:3 / 1080x1440 竖 3:4）—— 标题 + 信息块，适配 B站/抖音/小红书
...
```text
口播稿 → 句级切分 → 云希配音 → 句级时间轴
   → 章节/场景规划 → HyperFrames 构图 → lint/validate/inspect
   → 渲染 16:9 主视频 → 横竖封面 → 平台标题 → outputs/ 交付
```
...
```text
用 $tryworld-paper-video 把这篇口播稿做成视频，加上横竖封面和标题。
```
...
- 右下角单行水印 **试界TryWorld 纸上算法**
- 每 45-60 秒（或每章节开头）盖朱红**试界原创**印章
